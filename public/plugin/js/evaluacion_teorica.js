...
function carga_quiz(modulo, preguntas, tema){
	...
	var botones = "";
	var paneles = "";
	var n_preguntas = preguntas + 1;
	cont_preguntas = preguntas;

	for (i = n_preguntas; i < 26; i++) {
		$('#li_'+i).hide();
	}
	...
	if(modulo == 1){
		...
		//arreglo=ajax_quiz(modulo, tema, 5);
		...
		for (i = 1; i < n_preguntas; i++) {
			...
			//console.log(i, division, n_preguntas);
			tipo = ale = Math.round(Math.random()*(4-1)+parseInt(1));
			//tipo = 2;

			if (tipo == 1) {
				arreglo = ajax_quiz(modulo, tema, 1, division);
				var panel = tipo1(arreglo, modulo, tema, i);
				paneles += panel;
			}else if (tipo == 2) {
				arreglo = ajax_quiz(modulo, tema, 2, division)
				var panel = tipo2(arreglo, modulo, tema, i);
				paneles += panel;
			}else if (tipo == 3) {
				arreglo = ajax_quiz(modulo, tema, 3, division)
				var panel = tipo3(arreglo, modulo, tema, i);
				paneles += panel;
			}else if (tipo == 4) {
				arreglo = ajax_quiz(modulo, tema, 4, division)
		...
		divgraf.setAttribute('id', 'bar');
		divgraf.setAttribute('style', 'margin:0;');
		divgraf.setAttribute('class', 'progress progress-info progress-striped active');
		divgraf.innerHTML = '<div class="progress-bar progress-bar-striped active" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100" style="width: 0%;"></div>';
		parent.appendChild(divgraf);

		var botonesui = document.createElement("ul");
		botonesui.setAttribute('class', 'pager wizard col-lg-12');
		botonesui.innerHTML = '<li class="previous first"><a href="javascript:;">First</a></li><li class="previous"><a href="javascript:;">Previous</a></li><li class="next last"><a href="javascript:;">Last</a></li><li class="next"><a href="javascript:;">Next</a></li><li class="next finish" id="finaliza" style="display:none;"><a href="javascript:;" onclick="finalizar()">Finish</a></li>';
		parent.appendChild(botonesui);
		...
		for (var i = 1; i < n_preguntas; i++) {
			...
			console.log(i, division, n_preguntas, tipo);
		}
		...
		cargar_evaluacion(1, n_preguntas);
		pregunta();
	}else if(modulo == 22){
		...
		$('#panel_evaluacion').show();
		$('#prueba').hide();
		cargar_evaluacion(2, n_preguntas);
		pregunta();
	}else if(modulo == 33){
		...
		$('#panel_evaluacion').show();
		$('#prueba').hide();
		cargar_evaluacion(3, n_preguntas);
		pregunta();
	}
...
function ajax_quiz(modulo, tema, opc, division){
	panel = "";
