...
class formularios
{
	...
	{		
		...
		if($opc ==  5){/// Practico
			...
			$sql2="SELECT * FROM $modulo inner join descripciones de ON $modulo.id_descripcion = de.id_descripcion WHERE id != '".$id."' ORDER BY RANDOM() LIMIT 15";	
			...
			do{
				$sql2="SELECT * FROM $modulo inner join descripciones de ON $modulo.id_descripcion = de.id_descripcion WHERE id != '".$id."' ORDER BY RANDOM() LIMIT 1";	
				$resultado2 = ejecutar_sql($sql2);
				while ($fila = pg_fetch_array($resultado2))
				{
					$arregloListas[] = array(
